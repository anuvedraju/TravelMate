...
export function CreateDestinations() {
  db.transaction((tx) => {
    tx.executeSql(
      "CREATE TABLE IF NOT EXISTS destinations (destinationId INTEGER,name TEXT,destinationName TEXT,startDate TEXT,endDate TEXT,userId TEXT,FOREIGN KEY (userId) REFERENCES users(userId))",
...
export function InsertDestinations(destinationData, user) {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      tx.executeSql(
        "INSERT INTO destinations (destinationId,name, destinationName, startDate, endDate, userId) VALUES (?, ?, ?, ?, ?)",
        [
...
export function updateTask(taskName, destinationId, isCompleted) {
  db.transaction((tx) => {
    tx.executeSql(
      "UPDATE tasks SET isCompleted = ? WHERE taskName = ? AND destinationId = ?",
      [isCompleted, taskName, destinationId],
      (_, result) => {
        if (result.rowsAffected > 0) {
